...
void McuTrigmuxConfig(void)
{
	...
	TRGMUX_DRV_SetTrigSourceForTargetModule(INST_TRGMUX, TRGMUX_TRIG_SOURCE_FTM2_INIT_TRIG, TRGMUX_TARGET_MODULE_FTM3_HWTRIG0);
#else
	// Set initialization trigger for FTM3 from FTM0
	TRGMUX_DRV_SetTrigSourceForTargetModule(INST_TRGMUX, TRGMUX_TRIG_SOURCE_FTM0_INIT_TRIG, TRGMUX_TARGET_MODULE_FTM3_HWTRIG0);
#endif
}
...
void McuLpitConfig(void)
{
	...
	LPIT_DRV_Init(INST_LPIT0, &lpit1_InitConfig);
...
void McuFtmConfig(void)
{
	...
	FTM_DRV_Init(INST_FLEXTIMER_IC_1, &flexTimer_ic_1_InitConfig, &stateIc1);
	...
	FTM_DRV_Init(INST_FLEXTIMER_MC0, &flexTimer_mc0_InitConfig_0, &stateMc0);
	/* Stop FTM0 counter */
	//FTM_DRV_CounterStop(INST_FLEXTIMER_MC0);
	FTM_DRV_InitCounter(INST_FLEXTIMER_MC0, &flexTimer_mc0_TimerConfig_0);
...
void McuPdbConfig(void)
{
	...
	PDB_DRV_Init(INST_PDB_0, &pdb0_InitConfig0);
	/* PDB1 module initialization */
	PDB_DRV_Init(INST_PDB_1, &pdb1_InitConfig0);

	/* PDB0 CH0 pre-trigger0 initialization */
	PDB_DRV_ConfigAdcPreTrigger(INST_PDB_0, 0, &pdb0_AdcTrigInitConfig0);
	/* PDB1 CH0 pre-trigger0 initialization */
	PDB_DRV_ConfigAdcPreTrigger(INST_PDB_1,0, &pdb1_AdcTrigInitConfig0);
	/* PDB1 CH0 pre-trigger1 initialization */
    PDB_DRV_ConfigAdcPreTrigger(INST_PDB_1,0, &pdb1_AdcTrigInitConfig1);

	/* PDB0 modulus value set to half of the PWM cycle */
	PDB_DRV_SetTimerModulusValue(INST_PDB_0, HALF_PWM_MODULO);
    /* Set PDB1 modulus value set to half of the PWM cycle */
	PDB_DRV_SetTimerModulusValue(INST_PDB_1, HALF_PWM_MODULO);
	...
	PDB_DRV_SetAdcPreTriggerDelayValue(INST_PDB_0, 0, 0, PDB_DELAY_MIN);
	/* PDB1 CH0 pre-trigger0 delay set to sense DC bus current in the middle of the PWM cycle */
	PDB_DRV_SetAdcPreTriggerDelayValue(INST_PDB_1, 0, 0, 0);
	...
	PDB_DRV_SetAdcPreTriggerDelayValue(INST_PDB_1, 0, 1, PDB_DELAY_MIN);
