...
void PDB0_IRQHandler(void)
{
	...
	// Disable PDB0
	PDB_DRV_Disable(INST_PDB_0);

	/* Clear PDB0 sequence errors */
	PDB_DRV_ClearAdcPreTriggerSeqErrFlags(INST_PDB_0, 0, 0xFF);
...
void PDB1_IRQHandler(void)
{
	...
	// Disable PDB1
	PDB_DRV_Disable(INST_PDB_1);

	// Clear PDB1 sequence errors
	PDB_DRV_ClearAdcPreTriggerSeqErrFlags(INST_PDB_1, 0, 0xFF);
...
void FTM2_Callback()
{
	...
	SensorHall.Period[SensorHall.Sector] = FTM_DRV_GetInputCaptureMeasurement(INST_FLEXTIMER_IC_1, 1U);
...
// Freewheeling is ignored in Sensorbased mode
    ...
    LPIT_DRV_ClearInterruptFlagTimerChannels(0, 0x01);
